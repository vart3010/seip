...
class MonitoringController extends ResourceController
{
    ...
    public function showAction(Request $request){        
        ...
        $queryRepository = "createPaginatorStandardization";            
        //$repository = $em->getRepository("\Pequiven\SIGBundle\Entity\Tracing\Standardization");//->findBy(array('relationObject' => $id, 'typeObject' => $type));
        $criteria = $request->get('filter',$this->config->getCriteria());
        $sorting = $request->get('sorting',$this->config->getSorting());        
        
        $repository = $this->container->get('pequiven.repository.monitoring_sig');         
        
        $criteria['idManagemenSystems'] = $id;
        ...
        $apiDataUrl = $this->generateUrl('pequiven_sig_monitoring_list_standardization', $routeParameters);        
        
        $view = $this
            ->view()
            ->setTemplate($this->config->getTemplate('Tracing/list.html'))
            ->setTemplateVar($this->config->getPluralResourceName())
        ;
        $view->getSerializationContext()->setGroups(array('id','api_list'));
