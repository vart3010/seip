...
namespace Pequiven\SEIPBundle\Command;
...
class GenerateObjectAclCommand extends ContainerAwareCommand
{
    ...
    public function configure()
    {
        $this
            ...
            ->addOption('object_specific', null, InputOption::VALUE_OPTIONAL, 'If set, the task will ask for each admin if the ACLs need to be generated and what object owner to set, if any.')
        ;
    }
...
//        var_dump($input->getOption('object_specific'));
//        die();
        ...
        foreach ($this->getContainer()->get('sonata.admin.pool')->getAdminServiceIds() as $id) {
            ...
            if ($input->getOption('object_specific') !== NULL && $input->getOption('object_specific') != $id ) {
                continue;
            }
