...
class IndicatorController extends ResourceController {
    ...
    /**
     * Lista de IndicadoresSIG por nivel(Estratégico, Táctico u Operativo)
     * 
     * @param Request $request
     * @return type
     */
    function listsigAction(Request $request) {
        $level = $request->get('level');
        ...
        $view = $this
                ->view()
                ->setTemplate($this->config->getTemplate('sig/list.html'))
                ->setTemplateVar($this->config->getPluralResourceName())
        ;
        $view->getSerializationContext()->setGroups(array('id', 'api_list', 'valuesIndicator', 'api_details', 'sonata_api_read', 'formula'));
        if ($request->get('_format') == 'html') {
